...
do
   ...
   #   lupdate ../*.ui -ts uifiles.ts -source-language en_US -target-language $targetstr -no-obsolete
      ...
      lconvert -i title.ts pyfiles.ts -o Asm4.ts -source-language en_US -no-obsolete -sort-contexts -verbose
      # Join Asm4.ts to exist Asm4_(language).ts file ( -no-obsolete)
      lconvert -i title.ts Asm4.ts asm4_$lang.ts -o asm4_$lang-n.ts -source-language en_US -target-language $targetstr -no-obsolete -sort-contexts -verbose
      # (Release) Creation of *.qm file from Asm4_(language).ts
      lrelease asm4_$lang.ts
      # Delete unused files
      # rm uifiles.ts
      # rm pyfiles.ts
      # rm Asm4.ts
