...
class importDatumCmd():
    ...
    def GetResources(self):
        tooltip  = App.Qt.translate("Asm4_importDatum", "Imports the selected Datum object(s) from a sub-part into the root assembly.\n"
        ...
        return {"MenuText": App.Qt.translate("Asm4_importDatum", "Import Datum object"),
                "ToolTip" : tooltip,
                "Pixmap"  : iconFile }
    ...
    def Activated(self):
        ...
        if selTree:
            ...
            if len(selection)==2 and selection[0].getParentGeoFeatureGroup() == rootContainer:
                ...
                if selDatum.MapMode == 'Deactivated':
                    message = App.Qt.translate("Asm4_importDatum", 'This will superimpose ')+Asm4.labelName(selDatum)+App.Qt.translate("Asm4_importDatum", ' in ')+Asm4.labelName(rootContainer)+App.Qt.translate("Asm4_importDatum", ' on:\n\n')
                ...
                # selected datum is attached
                    ...
                    message = Asm4.labelName(selDatum)+App.Qt.translate("Asm4_importDatum", ' in ')+Asm4.labelName(rootContainer)+App.Qt.translate("Asm4_importDatum", ' is already attached to some geometry. ')
                    message += App.Qt.translate("Asm4_importDatum", 'This will superimpose its Placement on:\n\n')
                    for objName in selTree[1:-1]:
                        message += '> '+objName+'\n'
                    message += '> '+Asm4.labelName(selDatum)
            ...
            if len(selTree)<3:
                message = selDatum.Name + App.Qt.translate("Asm4_importDatum", ' is already at the top-level and cannot be imported')
                Asm4.warningBox(message)
                return
        ...
        if len(selection) == 1:
            message = App.Qt.translate("Asm4_importDatum", 'Create a new ')+selDatum.TypeId+App.Qt.translate("Asm4_importDatum", ' in ')+Asm4.labelName(rootContainer)+App.Qt.translate("Asm4_importDatum", ' \nsuperimposed on:\n\n')
            for objName in selTree[1:]:
                message += '> '+objName+'\n'
            message += App.Qt.translate("Asm4_importDatum", '\nEnter name for this datum :')+' '*40
            userSpecifiedName,confirm = QtGui.QInputDialog.getText(None,App.Qt.translate("Asm4_importDatum", 'Import Datum'),
                    message, text = proposedName)
        else:
            message = str(len(selection)) + App.Qt.translate("Asm4_importDatum", " selected datum objects will be imported into the root assembly\n"
