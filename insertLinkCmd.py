...
class insertLink():
    ...
    def GetResources(self):
        tooltip  = translate("Asm4_insertLink", "<p>Insert a Part into the assembly. " + \
        ...
        return {"MenuText" : translate("Asm4_insertLink", "Insert Part"),
                "ToolTip"  : tooltip,
        ...
        # if a broken link is selected
            ...
            if selObj.isDerivedFrom('App::Link') and selObj.LinkedObject is None:
                ...
                if parent.TypeId == 'App::Part' and parent.getParentGeoFeatureGroup() is None:
                    ...
                    self.rootAssembly = parent
                    self.origLink = selObj
                    self.UI.setWindowTitle(translate("Asm4_insertLink", 'Re-link broken link'))
                    self.insertButton.setText(translate("Asm4_insertLink", 'Replace'))
                    self.linkNameInput.setText(Asm4.labelName(selObj))
                    self.linkNameInput.setEnabled(False)

        if self.rootAssembly is None:
            Asm4.warningBox( translate("Asm4_insertLink", 'Please create an Assembly' ))
            return
    ...
    def openFile(self):
        ...
        dialog = QtGui.QFileDialog( QtGui.QApplication.activeWindow(),
                                    translate("Asm4_insertLink", "Select FreeCAD document to import part from" ))
        ...
        '''
        dialog.setNameFilter(translate("Asm4_insertLink", "Supported Formats *.FCStd *.fcstd (*.FCStd *.fcstd);;All files (*.*)"))
        if dialog.exec_():
    ...
    def onCreateLink(self):
        ...
        # only create link if there is a Part object and a name
            ...
            if App.ActiveDocument.FileName!='' or App.ActiveDocument==selectedPart.Document:
                ...
                Asm4.warningBox(translate("Asm4_insertLink", 'The current document must be saved before inserting an external part'))
                return
    ...
    def drawUI(self):
        ...
        self.UI.setWindowTitle(translate("Asm4_insertLink", 'Insert a Part'))
        ...
        # Cancel button
        self.cancelButton = QtGui.QPushButton(translate("Asm4_insertLink", 'Cancel'), self.UI)
        # Cancel button
        self.openFileButton = QtGui.QPushButton(translate("Asm4_insertLink", 'Open file'), self.UI)
        # Insert Link button
        self.insertButton = QtGui.QPushButton(translate("Asm4_insertLink", 'Insert'), self.UI)
        self.insertButton.setDefault(True)

        # Place the widgets with layouts
        self.mainLayout = QtGui.QVBoxLayout(self.UI)
        self.mainLayout.addWidget(QtGui.QLabel(translate("Asm4_insertLink", "Filter :")))
        self.mainLayout.addWidget(self.filterPartList)
        self.mainLayout.addWidget(QtGui.QLabel(translate("Asm4_insertLink", "Select Part to be inserted :")))
        self.mainLayout.addWidget(self.partList)
        self.mainLayout.addWidget(QtGui.QLabel(translate("Asm4_insertLink", "Name for the link :")))
